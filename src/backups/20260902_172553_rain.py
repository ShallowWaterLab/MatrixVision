...
def _bg_init(expected_seq, query):
    ...
    try:
        ...
        with _audio_init_lock:
            ...
            if engine.ready:
                audio = engine
                sys.stderr.write(f"♪ audio ready\n")
            elif engine.error:
                AUDIO_ENABLED = False
                sys.stderr.write(f"♪ audio error: {engine.error}\n")
...
def save_config(contrast, edge, fps=16, speed=1.0, audio=True, audio_query=""):
    try:
...
class RainGrid:
    ...
    def update(self, gray, speed_mult=1.0, audio_energy=0.0):
        rows = self.rows
        lc = self.logicalCols
        if gray is None or gray.shape[0] != rows or gray.shape[1] != lc:
            return
        cam = gray.astype(np.float32) / 255.0

        # Camera brightness tracks the live frame directly, no trail.
        for y in range(rows):
...
def draw_settings_overlay(term_rows, term_cols, selected, contrast, edge, audio_enabled=True, audio_query="", save_msg=None):
    header = f"MatrixVision  v{VERSION}"
    preset_names = [p[0] for p in MATRIXMIX_AUDIO_PRESETS]
    is_custom = audio_query not in [p[1] for p in MATRIXMIX_AUDIO_PRESETS]
    if is_custom:
        display_val = f"CUSTOM: {audio_query[:30]}"
    else:
        display_val = preset_names[[p[1] for p in MATRIXMIX_AUDIO_PRESETS].index(audio_query)]

    query_values = preset_names + ["CUSTOM"]
    query_idx = len(query_values) - 1 if is_custom else next((i for i, (_, url) in enumerate(MATRIXMIX_AUDIO_PRESETS) if url == audio_query), 0)

    items = [
        ("CONTRAST", ["0","1","2","3"], max(0, min(3, int(contrast)))),
        ("EDGE", ["0","1","2","3"], max(0, min(3, int(edge)))),
        ("AUDIO", ["OFF","ON"], 1 if audio_enabled else 0),
        ("AUDIO_QUERY", query_values, query_idx),
        ("SAVE", None, None),
        ("QUIT", None, None),
    ]
...
def run(cam_idx=0, cam_w=1280, cam_h=720):
    global CONTRAST_LEVEL, EDGE_LEVEL, AUDIO_ENABLED, save_message, save_message_until, audio, audio_query, _audio_init_seq
    backup_version()
    try:
        my_pid = os.getpid()
    ...
    try:
        while True:
            if not paused and not settings_open:
                gray = None
                if frame_idx % camera_interval == 0:
                ...
                if len(rem) >= 2 and rem[0] == '[' and rem[1] in ('A', 'B', 'C', 'D'):
                    if settings_open:
                        if rem[1] == 'A':
                            ...
                            if settings_selected in (0, 1, 2, 4, 5):
                                _apply_setting(settings_selected)
                            elif settings_selected == 3:
                                presets = MATRIXMIX_AUDIO_PRESETS
                                is_custom = audio_query not in [p[1] for p in presets]
                                if is_custom:
                                    audio_query = presets[-1][1]
                                else:
                            ...
                            if settings_selected in (0, 1, 2, 4, 5):
                                ...
                                if AUDIO_ENABLED:
                                    ...
                                    with _audio_init_lock:
                                        _audio_init_seq += 1
                                    _start_audio_async(audio_query)
                        draw_settings_overlay(term_rows, term_cols, settings_selected, CONTRAST_LEVEL, EDGE_LEVEL, AUDIO_ENABLED, audio_query, save_message if save_message else None)
                elif not rem:
                    if settings_open:
                        settings_open = False
                        init_term()
                        ...
                        settings_open = True
                        settings_selected = 0
                        draw_settings_overlay(term_rows, term_cols, settings_selected, CONTRAST_LEVEL, EDGE_LEVEL, AUDIO_ENABLED, audio_query, None)
                        try:
                            while True:
                                key_q.get_nowait()
                        except Exception:
            ...
            if settings_open:
                if first == '\x1b':
                    ...
                    if first == '\x1b[A':
                        ...
                        # Left: cycle AUDIO_QUERY backward
                        if settings_selected == 3:
                            presets = MATRIXMIX_AUDIO_PRESETS
                            is_custom = audio_query not in [p[1] for p in presets]
                            if is_custom:
                                # From CUSTOM, go to last preset
                                audio_query = presets[-1][1]
                            else:
                                for i, (name, url) in enumerate(presets):
                                    if url == audio_query:
                        ...
                        # Right: cycle AUDIO_QUERY forward
                        if settings_selected == 3:
                            presets = MATRIXMIX_AUDIO_PRESETS
                            is_custom = audio_query not in [p[1] for p in presets]
                            if is_custom:
                                # From CUSTOM, go to first preset
                                audio_query = presets[0][1]
                            else:
                                for i, (name, url) in enumerate(presets):
                                    if url == audio_query:
...
def _apply_setting(idx):
    global CONTRAST_LEVEL, EDGE_LEVEL, AUDIO_ENABLED, audio, save_message, save_message_until, _audio_init_seq, audio_query
    if idx == 0:
        ...
        if audio_query not in [p[1] for p in MATRIXMIX_AUDIO_PRESETS]:
            # CUSTOM input mode
            if AUDIO_ENABLED:
                try:
                    if audio is not None:
            ...
            try:
                ...
                pass
            while True:
                # Show input prompt on current screen
                sys.stdout.write(f"\x1b[{term_rows};1H\x1b[K\x1b[33mINPUT: {_custom_input}_\x1b[0m")
                sys.stdout.flush()
                try:
                    key = key_q.get(timeout=0.5)
                except Exception:
                    continue
                if key == '\x1b':
                    # ESC cancels - restart old audio if it was enabled
                    if AUDIO_ENABLED:
                        _start_audio_async(audio_query)
                    break
                elif key in ('\r', '\n'):
                    if _custom_input.strip():
                        audio_query = _custom_input.strip()
                        AUDIO_ENABLED = True
                        _start_audio_async(audio_query)
                    else:
                        if AUDIO_ENABLED:
                    ...
                    _custom_input = _custom_input[:-1]
                elif key.isprintable() and len(_custom_input) < 100:
                    _custom_input += key
            # Clear input line
            sys.stdout.write(f"\x1b[{term_rows};1H\x1b[K")
            sys.stdout.flush()
        else:
            # Preset - switch directly, always turn audio ON
            if AUDIO_ENABLED:
                try:
                    if audio is not None:
                        audio.stop()
        ...
        save_config(CONTRAST_LEVEL, EDGE_LEVEL, fps=MATRIXMIX_FPS, speed=MATRIXMIX_SPEED, audio=AUDIO_ENABLED, audio_query=audio_query)
        save_message = "✔ Saved"
