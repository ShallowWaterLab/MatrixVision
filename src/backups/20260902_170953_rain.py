...
def _bg_init(expected_seq):
    global audio, AUDIO_ENABLED
    try:
        engine = AudioEngine(query=audio_query)
...
def _start_audio_async():
    global _audio_init_seq
    with _audio_init_lock:
        seq = _audio_init_seq + 1
        _audio_init_seq = seq
    threading.Thread(target=_bg_init, args=(seq,), daemon=True).start()
...
def run(cam_idx=0, cam_w=1280, cam_h=720):
    global CONTRAST_LEVEL, EDGE_LEVEL, AUDIO_ENABLED, save_message, save_message_until, audio
    backup_version()
    try:
        my_pid = os.getpid()
                ...
                if len(rem) >= 2 and rem[0] == '[' and rem[1] in ('A', 'B', 'C', 'D'):
                    if settings_open:
                        if rem[1] == 'A':
                            ...
                            # Left: adjust current item or cycle AUDIO_QUERY
                            ...
                            # Right: adjust current item or cycle AUDIO_QUERY
                            if settings_selected in (0, 1, 2, 4, 5):
                                _apply_setting(settings_selected)
                            elif settings_selected == 3:
                                presets = MATRIXMIX_AUDIO_PRESETS
...
def _apply_setting(idx):
    ...
    if idx == 0:
        ...
        # AUDIO_QUERY: just switch audio to the currently selected query
        if AUDIO_ENABLED:
            try:
                if audio is not None:
                    audio.stop()
