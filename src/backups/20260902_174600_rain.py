...
def draw_settings_overlay(term_rows, term_cols, selected, contrast, edge, audio_enabled=True, audio_query="", save_msg=None):
    ...
    items = [
        ...
        ("AUDIO_QUERY", preset_names, next((i for i, (_, url) in enumerate(MATRIXMIX_AUDIO_PRESETS) if url == audio_query), 0)),
        ("CUSTOM", None, None),
        ("SAVE", None, None),
        ("QUIT", None, None),
    ]
...
def run(cam_idx=0, cam_w=1280, cam_h=720):
    global CONTRAST_LEVEL, EDGE_LEVEL, AUDIO_ENABLED, save_message, save_message_until, audio, audio_query, _audio_init_seq, _custom_input, term_rows, term_cols
    backup_version()
    try:
        my_pid = os.getpid()
...
def _apply_setting(idx):
    global CONTRAST_LEVEL, EDGE_LEVEL, AUDIO_ENABLED, save_message, save_message_until, _audio_init_seq, audio_query, _custom_input, term_rows, term_cols
    if idx == 0:
        ...
        # CUSTOM input mode - stop current audio and let user type
        if AUDIO_ENABLED:
            try:
                if audio is not None:
                    audio.stop()
