...
audio_status = ""
_audio_status_event = threading.Event()
CAMERA_ENABLED = True
    ...
    for row_idx, (label, values, cur) in enumerate(items):
        ...
        if values is not None:
            ...
            if row_idx == 4 and audio_status:
                line = f"{marker}{label}: [ {cur_val} ]  \x1b[33m♪ {audio_status}\x1b[0m"
            else:
...
def run(cam_idx=0, cam_w=1280, cam_h=720):
    ...
    try:
        while True:
            ...
            if settings_open and audio_status == "connecting..." and audio is not None and getattr(audio, 'ready', False):
                audio_status = "ready"
                _audio_status_event.set()

            if settings_open and _audio_status_event.is_set():
                _audio_status_event.clear()
