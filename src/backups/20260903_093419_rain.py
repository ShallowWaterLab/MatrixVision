...
def draw_settings_overlay(term_rows, term_cols, selected, contrast, edge, audio_enabled=True, camera_enabled=True, audio_query="", save_msg=None):
...
def run(cam_idx=0, cam_w=1280, cam_h=720):
    global CONTRAST_LEVEL, EDGE_LEVEL, AUDIO_ENABLED, save_message, save_message_until, audio, audio_query, _audio_init_seq, CAMERA_ENABLED
    backup_version()
    try:
        my_pid = os.getpid()
                    ...
                    if settings_open:
                        if rem[1] == 'A':
                            ...
                            if settings_selected in (0, 1, 2, 5, 6):
                                ...
                                if AUDIO_ENABLED:
                                    _apply_setting(4)
                        draw_settings_overlay(term_rows, term_cols, settings_selected, CONTRAST_LEVEL, EDGE_LEVEL, AUDIO_ENABLED, CAMERA_ENABLED, audio_query, save_message if save_message else None)
                elif not rem:
                    if settings_open:
                        settings_open = False
                        init_term()
                        ...
                        settings_open = True
                        settings_selected = 0
                        draw_settings_overlay(term_rows, term_cols, settings_selected, CONTRAST_LEVEL, EDGE_LEVEL, AUDIO_ENABLED, CAMERA_ENABLED, audio_query, None)
                        try:
                            while True:
                                key_q.get_nowait()
                        except Exception:
...
def _apply_setting(idx):
    global CONTRAST_LEVEL, EDGE_LEVEL, AUDIO_ENABLED, save_message, save_message_until, _audio_init_seq, audio_query, CAMERA_ENABLED
    if idx == 0:
